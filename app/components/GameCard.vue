...
<template>
  <article class="overflow-hidden rounded-card border border-line bg-surface-1">
    <NuxtLink :to="localePath(`/games/${game.slug}`)" class="block focus-visible:outline-2">
      ...
      <div
        v-else
        class="flex aspect-video w-full items-center justify-center bg-surface-2 text-sm text-fg-2"
      >
        {{ t('catalog.noCover') }}
      </div>
      <div class="p-3">
        <h3 class="font-semibold leading-snug text-fg">{{ game.name }}</h3>
        <p v-if="game.released" class="mt-1 text-sm text-fg-2">
          {{ formatDate(game.released) }}
        </p>
        <p class="mt-2 flex flex-wrap items-center gap-2 text-xs text-fg-2">
          <span
            v-if="game.metacritic"
            class="font-numeric rounded-chip bg-surface-2 px-1.5 py-0.5 font-semibold text-fg"
          >
            {{ game.metacritic }}
          </span>
