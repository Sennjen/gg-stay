...
| `f:genre:{id}` `f:platform:{id}` `f:store:{id}` `f:mode:{X}` `f:age:{X}` `f:year:{YYYY}` `f:playtime:{X}` `f:loc:text` `f:loc:audio` `f:free` `f:ua` `f:priced` | set         | Facets                                                                                                                                                                                                                                        |
| `s:popularity` `s:rating` `s:metacritic` `s:released` `s:name` `s:price` `s:discount`                                                                           | sorted set  | Sort orders and ranges                                                                                                                                                                                                                        |
...
`s:name` scores are the rank of the name in locale-aware order, computed by the job. Games without a price are absent from `s:price` and `s:discount`, so price sorts never list them.
...
Text search combined with an index filter is a case-insensitive substring match over indexed names, applied to the ordered id list before paging; Upstash has no full-text module and 3 000 names do not need one.
